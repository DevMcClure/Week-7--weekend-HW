...
function App() {
  ...
  const itemNodes = items.map((item, index) => {
    return(
      <li key={index} className = {item.isPurchased ? "purchased":"not-purchased"}><span>{item.name}</span>
      {item.isPurchased ? <span className = "purchased">Purchase</span>: <button onClick={() => purchaseItem(index)}> Purchase</button>}
      </li>
    )
  })
